"""Verify the recorded carrier-boundary intervention and score paired training subsets.

Run from the Enricher repository at the report's tested SHA:
python3 compare-carrier-boundaries.py BASELINE_LIVE_DIR VARIANT_LIVE_DIR NEW_PRIVATE_DIR
...
intervention = read(Path(__file__).with_name('carrier-boundaries-intervention.json'))
...
assert {p['policy_version'] for p in baseline['predictions']} == {p['policy_version'] for p in variant['predictions']} == {'jev-policy-v3'}
...
for entry in new_calls:
    ...
    assert new['questions']['carriers'] != old['questions']['carriers']
    restored['questions']['carriers'] = copy.deepcopy(old['questions']['carriers'])
    assert restored == old
    question_changes += 1
...
summary = {
    'intervention': 'only carriers question criteria changed; observed structure precedence and distinct insufficient/no-match outcomes',
    'verified_pairs': 42,
    ...
    'model': 'jev-1.13.0',
    'policy': 'jev-policy-v3',
    'new_calls': old_live['calls'] + live['calls'],
    'historical_calls_including_smoke': 127,
    'cumulative_calls': 127 + old_live['calls'] + live['calls'],
    'baseline_input_tokens': old_live['input_tokens'],
    'baseline_p50_latency_ms': old_live['p50_latency_ms'],
    'baseline_p95_latency_ms': old_live['p95_latency_ms'],
    'baseline_reserved_input_tokens': old_live['reserved_input_tokens'],
    'baseline_input_price_estimate_usd': old_live['input_tokens'] * 0.042 / 1_000_000,
    ...
    'limitations': ['7 independent authored training groups; translations/variants are not independent',
                    'fixed baseline-then-variant order; no repeated run to estimate provider/temporal variation',
