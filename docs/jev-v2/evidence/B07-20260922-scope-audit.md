...
依据 S #31 原 B07-T01–T10 与 R2/R3、`07-android-compat.md`，审查 Share `3a891f1eceda116b5c76007b8887fde8d36e0dd5` / Enricher `86b82b5e0ccf0096cd67445ce80e02c76f89b505`。状态是逐条证明程度，不用已有门禁或历史摘要代替要求。
...
| B07-T02 能力、summary/detail、representation/revision 缓存 | 正常列表 summary 与详情按需；本批 v2 taxonomy 账号切换清空并拒绝旧账号异步响应；设置页 Token 入口同步清空 v2 状态，持久化监听不再比较乐观 UI token | `refreshLinks` 不使已有 v2 selection 失效；`ensureLink` 的旧账号结果仍可写回，账号切换保留原 links/search；缺少 representation/schema/revision 的完整缓存身份。单个 v2Available 可被不同接口结果覆盖 | 修复账号/材料/整理/schema 变化后的所有读取缓存与异步回调；独立协议能力，重测旧 Worker 和 flag off |
...
| B07-T05 离线/冲突/账号隔离 | R3-07 三阶段，新增四阶段：完整 credential/server fingerprint、显式旧队列恢复、缺失旧 CAS 冲突、两次真实跨端写入、进程重启和中途失败 | 队列归属已加强，但整体账号读取缓存仍见 T02；本批跨客户端是实际 HTTP 写入，不是完整 Web 浏览器 UI 竞争 | 补齐 T02，并在完整 App/Web 用户流程重测跨端冲突和未确认草稿 |
...
| B07-T08 导航/恢复/分页/可访问 | ViewModel/DataStore 强制进程恢复、原动作串行、常规设备流程 | `DimensionRow` 是普通横向 Row，完整词表没有 wrap/横向滚动；大字体/小屏/触控和全词表可达性未证明。账号切换导航载入仍依赖 id effect | 完整词表小屏/大字体设备测试、旋转和后台恢复；原图片/译文回归 |
...
| B07-T10 CI/设备/证据与交付 | 本地 strict unit/lint/build、实际 API26；7 阶段授权 Worker/D1 设备 harness；Go/Worker 全门禁。device workflow 已接 API26/35 真业务链并上传日志/XML | 最终 S `3a891f1` 远端 API26/35 矩阵均成功，各 13 普通 + 7 真实 Worker 阶段，详见本批报告；物理真机未执行，且整张表仍有明确缺口 | 固定 SHA 的 XML 与七阶段 artifacts 已逐项核对；随后完成本表剩余项与独立复审，不以局部通过关闭 S #31 |
