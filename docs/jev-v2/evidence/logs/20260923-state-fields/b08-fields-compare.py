"""Verify the recorded two-field intervention and score paired training subsets.

Run from the Enricher repository at the report's tested SHA:
python3 b08-fields-compare.py BASELINE_RECOVERY_DIR VARIANT_LIVE_DIR NEW_PRIVATE_DIR
...
baseline = read(baseline_root / 'dataset.json')
journal = read(baseline_root / 'recovery.json')
...
assert journal['reference_hash'] == live['plan']['reference_hash']
...
old_requests = {}
for root in sorted({call['source_directory'] for call in journal['calls']}):
    for entry in lines(Path(root) / 'calls.ndjson'):
...
for entry in new_calls:
    ...
    for name, question in restored['questions'].items():
        assert '`primary`' in question['instructions'] and '`context`' in question['instructions']
        question['instructions'] = question['instructions'].replace('`primary`', '`original_text`').replace('`context`', '`context_text`')
...
summary = {
    'intervention': 'only original_text/context_text instruction names replaced with actual primary/context names',
    'verified_pairs': 42,
    'verified_question_replacements': question_changes,
    'all_actual_request_states_identical': True,
    'reference_hash': journal['reference_hash'],
    'model': 'jev-1.13.0',
    'policy': 'jev-policy-v2',
    'new_calls': live['calls'],
    'historical_calls_including_smoke': 43,
    'cumulative_calls': 43 + live['calls'],
    'baseline_input_tokens': journal['reported_input_tokens'],
