...
载体仍为单值，判断范围是本次实际提供的来源材料。按下列顺序解决共存结构：

1. 已提供且确认与原帖关联的同作者续帖正文 → `author_continuation`，即使同时有外链正文。
2. 否则，已提供原帖所链接的外部文章正文 → `external_article`，不能因入口仍有一条原帖而改选 single。
3. 否则，可观察材料为原帖 → `single`；引用、他人评论和普通链接不等于作者续帖或外链正文。
4. 正文不足或来源关系无法确认 → `unknown`。可观察但不属于现有载体的独立书籍/视频等 → 问题的 `none`，保留合法空结果。

只有 URL/标题不算已获取文章正文，只有 1/n 字样不算已经提供续帖。以上不声称抓取完整，也不从主题或文章长度猜来源身份。若未知角色的材料阻止判断结构，应选择 unknown。

Worker `definition_version=2` 的变更限于载体定义/边界；旧 ID、显示名及 v1 的 17/7/5 词表保留。Go 的新载体问题分开 unknown 和 none；其他问题与 policy 不变。定义语义改变生成新 spec ID/hash，新旧不可互相冒充。历史注册对象、raw、决定和人工选择不重写；启用新分类必须明确切换到审核后的目标，回退选择旧目标及对应旧消费者，不用新编译器覆盖旧 spec。同名历史分类按其原 spec 解释。
