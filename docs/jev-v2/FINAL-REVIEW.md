> 2026-09-23 B05 最新局部证据：[实际多维筛选、实体状态、计数和缓存](evidence/B05-20260923-filter.md)。受测 S `aad7d0a`；两真实列表入口已接入权威有效值，同维 OR/跨维 AND，非法条件拒绝，计数按筛选条件，0025 修复来源变化缓存。完整原范围仍未完成，Android 无关键词收藏库的 v1 本地过滤另行修复；远端门禁与规模限制见报告。
