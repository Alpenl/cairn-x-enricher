...
> 2026-09-23 实体身份：[有限 canonical、明确 unknown 与逐处来源](evidence/B09-20260923-canonical.md)。S `5c5d583` / E `dd6bcfa`；生产 Choice + 受控目录 + 私有持久 observations，两个同名分别保留身份，无依据不合并。276 Worker、完整 Go、19 实际服务、42 实际浏览器、57 页面夹具通过；新增付费 0。Android 逐项来源、真实质量、全历史/治理及独立复审继续，完整 235 项保留。

> 2026-09-23 来源版本修复：[复合保存只递增一次与迁移兼容](evidence/B03-20260923-source-revision.md)。S `0ae27c8` / E `57e1e5f`；旧实际接口 2 → 4 的失败已修复，三次实际 Go 保存为 2 → 5，重复保存身份不变。271 Worker、完整 Go、18 实际服务、40 实际浏览器和 57 页面夹具通过。零新增付费；完整 235 项及独立复审继续。

> 2026-09-23 实体持久结果：[跨进程复用、丢响应恢复与来源链接失效](evidence/B09-20260923-entity-cache.md)。S `8f4f0ec` / E `8db9f3f`；实际旧 CLI 重复调用对照失败，新五进程场景仅 2 次本地实体 HTTP，人工优先及预算耗尽 hit 保留。257 Worker、完整 Go、17 实际服务、40 浏览器检查通过；新增付费 0，累计 211。完整 235 项及 canonical/质量/独立复审继续。

> 2026-09-23 普通分类预算：[逐实际调用持久授权与双向兼容检查](evidence/B04-20260923-classification-budget.md)。S `7f02a92` / E `6142692`；UTC 日限全局 20 / 逐条 5、拆批逐次计量、丢授权零推断、耗尽不继续消耗 attempt。247 Worker、完整 Go、16 实际服务与 40 浏览器检查通过；新五进程场景 2 次本地 HTTP、零付费，累计 211。实体去重及完整 235 项范围继续。

> 2026-09-23 重排缓存修复：[持久结果、并发去重与版本失效](evidence/B09-20260923-rerank-cache.md)。S `47662e3` / E `b6f9043`；实际生产入口接入私有 D1 缓存、完整请求与候选版本、固定期限、删除清理和当前筛选页复读。238 Worker、完整 Go 门禁、15 个实际服务场景通过；7 个独立进程验证重启复用和丢响应恢复。实体结果去重、普通分类整体预算和完整 235 项范围继续；新增付费 0，累计 211。

> 2026-09-23 扩展预算修复：[持久 D1 授权、并发限额与跨进程恢复](evidence/B09-20260923-budget.md)。S `305d0d8` / E `796a508`；实体、补材料与重排在真实入口共享逐条/全局预算与 deadline。原总上限 1 的探针现实际调用 1；五个独立 Go 进程和 14 个真实跨仓场景通过，228 Worker 与完整 Go 门禁通过。相同查询结果缓存、普通分类整体预算及完整原范围继续；新增付费 0，累计 211。

> 2026-09-23 全范围盘点：[235 个编号与逐任务证据边界](evidence/B10-20260923-scope.md)。固定基线 E `7936be0` / S `c2091fa`；126 任务、39 R、30 SC、14 F、14 R2、12 R3 全保留，112 个固定版本文件链接校验通过。生产重排连续两次请求在总上限 1 下实际调用 2，已记录 G01 失败；其余 canonical、词表治理、保留期、Android、迁移兼容、自动质量及独立复审缺口继续。新增付费 0，累计 211；此索引不代表功能或质量接受。

> 2026-09-23 NAS 图片兼容修复：[私有缓存、旧后端孤儿读取与真实浏览器](evidence/B10-20260923-nas-image.md)。E `86be050` / S `c2091fa`；Go 取图前后检查所属收藏，NAS no-store，生产 URL 避开旧缓存键。完整 make verify、13 实际服务、12 缓存浏览器及 40 完整浏览器检查通过；旧夹具 policy-v2 等待失败与修正记录保留。无新增付费，累计 211 次；完整原范围与独立复审继续。
...
> 2026-09-23 载体边界对照：[生产词表与真实两臂训练实验](evidence/B08-20260923-carriers.md)。S `f7ed68e` / E `d4d0e3b`；生产定义 v2，真实 API 与实验词表一致，仅载体问题改变。两臂各 42 次，载体 F1 0.8974 → 1，潜在用途 F1 略降；7 独立组，train-only，未晋升。210 Worker、完整 Go 门禁与 12 个实际服务场景通过。新增 84 次、累计 211 次，完整原范围保留。

> 2026-09-23 独立 CLI 修复：[多维词表与实际二进制验收](evidence/B04-20260923-cli-catalog.md)。E `d451e21` / S `9571445`；服务与命令共用 v2 优先词表，仅 404/405 回退。完整 Go 门禁、命令边界与 11 个实际服务场景通过；实际二进制验证限量分类、空队列零调用和人工值保留。本批零付费，累计 127 次，完整原范围继续。

> 2026-09-23 客观/个人边界修复：[新候选、版本化 policy 与真实 Worker 写入守卫](evidence/B04-20260923-personal-intent.md)。S `9571445` / E `8dced00`；默认 v3 不再从客观 raw 产出 contra，Worker 拒绝旧/新自动写入口绕过，显式人工值和历史审计保留。210 Worker、完整 Go 门禁与 10 个实际服务场景通过；本批零付费，累计 127 次。新 spec 质量尚未验证，完整原范围保留。

> 2026-09-23 最新 B08 实验：[来源角色合同与真实训练负结果](evidence/B08-20260923-source-roles.md)。E `315f7d9`；42 对请求只改共享角色描述。召回 89.05% → 87.14%、错误接受 5.08% → 6.63%，载体 F1 未改善，输入成本上升；工程门禁/9 个实际服务/源码 CI 通过，但质量未验收。新增 42 次、累计 127 次；train-only，参考/门槛不变。后续备注隔离和载体词条边界独立验证，完整原范围保留。

> 2026-09-23 最新 B08 受控实验：[实际输入字段名修复与 42 条训练对比](evidence/B08-20260923-state-fields.md)。E `97b9e32`；实际请求验证仅两个字段名变化，参考和 policy 不变。召回上升，但载体/潜在用途 F1 下降、完整字段覆盖下降，未晋升、未通过完整质量验收。本地完整门禁、9 个实际服务场景和源码 CI 通过。新增 42 次，累计 85 次；dev/holdout 未用于模型、拟合或质量评分。后续来源角色和备注隔离独立验证，完整原范围保留。
