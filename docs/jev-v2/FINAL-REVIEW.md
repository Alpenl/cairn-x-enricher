> 2026-09-22 B07 最新局部证据：[单标签重置、实体搜索与历史投影升级](evidence/B07-20260922-reset.md)。受测 S `a3794e4` / E `5de7999`；186 Worker、70 Android unit、33 共享向量、95 历史升级记录、8 真实跨仓及本地 API26 18 普通 + 9 真实阶段通过；两仓代码 CI 成功。远端设备结果见报告。完整原范围仍未完成，实际 v2 筛选未接入、缓存竞争与完整端到端矩阵继续保留。前批 [状态合同](evidence/B07-20260922-state.md) 证据继续有效。
