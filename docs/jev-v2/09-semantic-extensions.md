...
回滚消费者前关闭扩展 flag，保留迁移和兼容清理 Worker。将全局 `TYPESAFE_MODEL` 从 alias 改为固定版本也会改变普通分类目标，应按 B01 desired target/new generation 受控操作，不自动激活。普通分类持久预算另见 B04；实体结果去重、固定候选的真实相关性质量和逐 flag 收益仍待其余任务完成；本次工程验证不关闭整个 G01/B09，也不选择上线策略。
