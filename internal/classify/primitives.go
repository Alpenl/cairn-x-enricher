...
// ScoreAnswer is the typed transport form of an ordinal Score.
...
// UnmarshalJSON decodes an answer from the provider wire format. The provider
// may inline the primitive fields at the top level (the historical shape) or
// nest them; both are accepted, but the declared type must match the payload.
func (a *RawAnswer) UnmarshalJSON(data []byte) error {
	...
	case TypeScore:
		if wire.Score == nil || len(wire.Levels) == 0 || wire.Probabilities == nil {
			return errors.New("score answer is missing its level or distribution")
		}
		a.Score = &ScoreAnswer{Score: *wire.Score, Levels: wire.Levels, Probabilities: wire.Probabilities}
	default:
...
// ValidateProbabilityMap checks that the distribution is non-empty, covers the
// allowed options exactly, and sums to one within a small tolerance.
